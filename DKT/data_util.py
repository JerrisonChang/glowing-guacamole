...
def load_dataset(fn, batch_size=1, shuffle=True, num_kc=None):
    df = pd.read_csv(fn)

    if num_kc != None:
        skills = pd.unique(df['KC (Original)'])[:num_kc]
        df = df[df['KC (Original)'].isin(skills)]

    # Step 1.1 - Remove questions without skill
    df.dropna(subset=['KC (Original)'], inplace=True)

    # Step 1.2 - Remove users with a single answer
    df = df.groupby('Anon Student Id').filter(lambda q: len(q) > 1).copy()

    # Step 2 - Enumerate skill id
    df['skill'], _ = pd.factorize(df['KC (Original)'], sort=True)
...
def split_dataset(dataset, total_size, test_fraction):
